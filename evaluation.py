...
def all_micro(yhatmic, ymic):
  return micro_accuracy(yhatmic, ymic), micro_precision(yhatmic, ymic), micro_recall(yhatmic,
                                                                                     ymic), micro_f1(yhatmic, ymic)
...
if __name__ == "__main__":
  if len(sys.argv) < 5:
    print(
      "usage: python " +
      str(os.path.basename(__file__) + " [train_dataset] [|Y| (as string)] [version (mimic2 or mimic3)] [model_dir]")
      )
    sys.exit(0)
